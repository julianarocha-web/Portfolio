...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!wrapper || !boxes.length || typeof gsap === "undefined") return;
...
loop.toIndex(0, { duration: 0 });
  ...
  // Botones
  const toggleBtn = document.querySelector(".carrousel-controls .toggle");
  const nextBtn = document.querySelector(".carrousel-controls .next");
  const prevBtn = document.querySelector(".carrousel-controls .prev");

  toggleBtn &&
    toggleBtn.addEventListener("click", () => {
      wrapper.classList.toggle("show-overflow");
      wrapper.style.overflow = wrapper.classList.contains("show-overflow")
        ? "visible"
        : "hidden";
    });
